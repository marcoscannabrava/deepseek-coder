...
@click.command()
@click.option('--logpath', default='/tmp/deepseek_coder.log', help='Path to the log file.')
def run(logpath):
    print(f"{yellow}Starting DeepSeek Coder. Please wait...{nc}")
    vllm_logs = open(logpath, 'w+')
    ...
    while True:
        ...
        if "RuntimeError" in line:
            ...
            print(f"{yellow}See Logs: {logpath} {nc}")
            vllm_process.kill()
            exit(1)
        if time.time() - start > LONG_RUNNING_WARNING and not shown_long_running_disclaimer:
            ...
            print(f"Check progress by running `tail -f {logpath}`")
